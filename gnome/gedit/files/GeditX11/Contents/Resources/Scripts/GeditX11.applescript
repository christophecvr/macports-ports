...
on run
	...
	if envPWD is not equal to {} and envPWD is not equal to "" then
		...
		if AppArgsNumbers is greater than 1 then
			...
			do shell script "export PATH=/opt/local/bin:/opt/local/sbin:$PATH;exec gedit" & " " & AppXargs & " &> /dev/null &"
			set text item delimiters to linefeed
		else
			-- open command used no file or argument set
			do shell script "export PATH=/opt/local/bin:/opt/local/sbin:$PATH;exec gedit &> /dev/nul &"
		end if
		--else gedit is launched by click on the application in finder
	else
		do shell script "export PATH=/opt/local/bin:/opt/local/sbin:$PATH;exec gedit &> /dev/null &"
	end if
end run
	...
	if envPWD is not equal to {} and envPWD is not equal to "" then
		...
		repeat with I in SelFiles
			...
			if AppArgsNumbers is greater than 1 then
				...
				do shell script "export PATH=/opt/local/bin:/opt/local/sbin:$PATH;exec gedit" & " " & aSelFile & " " & AppXargs & " &> /dev/null &"
				set text item delimiters to linefeed
			else
				-- open command used in terminal with one or more files. No arguments.
				do shell script "export PATH=/opt/local/bin:/opt/local/sbin:$PATH;exec gedit" & " " & aSelFile & " &> /dev/null &"
			end if
		end repeat
